...
void Input::Update(MSG* msg)
{
	...
	case WM_KEYDOWN:
		...
		//for(InputObserver* observers : observerArray[msg->wParam]){
		//	observers->keyDown(msg->wParam);
		//}

		//for (int i = 0; i < KEYARRAYSIZE; i++) {
		//	if (m_keyArray[i]) {
		//		for (std::vector<InputObserver*> observers : observerArray) {
		//			for (InputObserver* obsever : observers) {
